var app = ( function() {
	...
	function initSpheres(model, fs) {
		let y = .8;
		let dAngle = 1 / 2 * Math.PI;
		let angle = dAngle;
		let x = 1
		let z = 1;
		createModel(model, fs,  [ 0.23, 0.65, 0.76, 0.49 ], [x, y, z], [0, 0, 0], [.15, .15, .15]);
		let modelObject = models[models.length - 1];
		modelObject.orbitAngle = angle;
		spheres.push(modelObject);

		angle += dAngle
		x = 2;
		z =	0;
		createModel(model, fs,  [ 1, 0.2, 0.45, 0.45 ], [x, y, z], [0, 0, 0], [.15, .15, .15]);
		modelObject = models[models.length - 1];
		modelObject.orbitAngle = angle;
		spheres.push(modelObject);
		...
		x = 1;
		z = -1;
		createModel(model, fs,  [ 0.56, 0.48, 0.28, 0.20 ], [x, y, z], [0, 0, 0], [.15, .15, .15]);
		modelObject = models[models.length - 1];
		modelObject.orbitAngle = angle;
		spheres.push(modelObject);
		...
		x = 0;
		z = 0;
		createModel(model, fs,  [0.192,0.141,0.314, 0.6], [x, y, z], [0, 0, 0], [.15, .15, .15]);
		modelObject = models[models.length - 1];
		modelObject.orbitAngle = angle;
		spheres.push(modelObject);
	}
		...
		window.onkeydown = function(evt) {
			...
			switch(c) {
				...
				case('Z'):
					...
					break;
				case('K'):
					spheres.forEach(model => model.orbitAngle += deltaRotate);
					spheres.forEach(model => {
						model.translate[0] += 0.091 * Math.sin(model.orbitAngle);
						model.translate[2] += 0.091 * Math.cos(model.orbitAngle);
					})
					break;
			}
